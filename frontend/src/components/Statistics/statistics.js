  ...
  return (
    <div className="stats-container">
      ...
      <div className="card-container">
        {exercisesData.map((entry, index) => (
          entry.exerciseType === 'Running' ||
          entry.exerciseType === 'Cycling' ||
          entry.exerciseType === 'Swimming' ||
          entry.exerciseType === 'Walking' ? (
                ...
                <div>
                  ...
                  <div>
                    <MText className="lead" mt={30}>
                      {entry.topSpeed !== null ? (
                        <span>
                          {entry.topSpeed} <span className="unit">km/hr</span>
                        </span>
                      ) : (
                        'N/A'
                      )}
                  ...
                  <Group mt="lg">
                    <div>
                      <MText className="label">
                        {entry.averagePace != null ? (
                          <span>
                            {entry.averagePace.toFixed(2)} <span className="unit">min/km</span>
                          </span>
                        ) : (
                          'N/A'
                        )}
                    ...
                    <div className="average-speed">
                      <MText className="label">
                        {entry.averageSpeed != null ? (
                          <span>
                            {entry.averageSpeed.toFixed(2)} <span className="unit">km/hr</span>
                          </span>
                        ) : (
                          'N/A'
                        )}
